...
class MarketTrends(BaseModel):
    ...
    trend_direction: str  # "appreciating", "stable", "depreciating"
...
class ValuationResponse(BaseModel):
    ...
    estimated_value: float = Field(..., ge=0, description="Estimated property value")
    estimated_value_range_low: float = Field(..., ge=0)
    estimated_value_range_high: float = Field(..., ge=0)
    confidence_score: float = Field(..., ge=0, le=100, description="Confidence in valuation")
...
class InstantValueResponse(BaseModel):
    ...
    value_range: Dict[str, float] = Field(..., description="Min and max value range")
    ...
    vs_zip_median: Optional[float] = Field(None, description="Comparison to zip code median")
    vs_city_median: Optional[float] = Field(None, description="Comparison to city median")
    
    # Market indicators
    market_trend: Optional[str] = None  # "up", "down", "stable"
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "estimated_at": "2024-01-15T10:30:00",
                "price_per_sqft": 250,
                "vs_zip_median": 1.05,
                "vs_city_median": 0.92,
                "market_trend": "up",
