...
# ─── CALLBACK HANDLER ──────────────────────────────────────
# ✅ FIX: Properly handles Safaricom callback structure with safety checks
...
def handle_mpesa_callback(callback_data: Dict[str, Any], client_ip: str = None) -> Dict[str, Any]:
    ...
    ✅ FIX: Uses .get() for safe access
    ✅ FIX: Validates structure before processing
    ✅ FIX: Always returns safe response
    """
    try:
        logger.info("=" * 60)
